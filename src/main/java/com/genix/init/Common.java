...
public class Common {
	 ...
	 */
	private static WebElement findElement(WebDriver driver, String elementName) {
		String locator;
		locator = elementName;
		int count = 0;
		while (count < 4) {
	...
	private static void moveToObjectelement(WebDriver driver, String xpath) {
		driver.switchTo().frame(driver.findElement(By.xpath(xpath)));
	}

	private static String getNextDate(String format, int currentToNextDate) {
	 ...
	 * @param driver the driver
	 */
	private static void scrollUpToBottom(WebDriver driver) {
		JavascriptExecutor js = ((JavascriptExecutor) driver);
		js.executeScript("window.scrollTo(0, document.body.scrollHeight)");
	}

	private static void scrollUpToElement(WebDriver driver, WebElement element) {
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", element);
	}

	private static void scrollUpTop(WebDriver driver) {
		JavascriptExecutor js = ((JavascriptExecutor) driver);
	...
	private static void scrollToHorizontal(WebDriver driver, WebElement element) {
	 ...
	 */
	// js executor
	private static void scrollToVertical(WebDriver driver, WebElement element) {
		Actions dragger = new Actions(driver);
		WebElement draggablePartOfScrollbar = element;
		// drag downwards
		int numberOfPixelsToDragTheScrollbarDown = 50;
	...
	private static void logcase(String msg) {
	...
	private static void movetoalertAndAccept(WebDriver webDriver) {
		try {
	...
	private static String getalertText(WebDriver webDriver) {
		try {
			Alert alert = webDriver.switchTo().alert();
			alerttext = alert.getText();
		} catch (Exception e) {
	...
	private static String getBeforeAndAfterDateMonthWiseForSingleDigit(int i) {
	...
	private static void acceptUnhandledAlert(WebDriver webDriver) {
		try {
	 ...
	 */
	private static void openMailinator(WebDriver driver, String emailAddress) {
		pause(3);
	...
	private static String getCurrentTimeStampString2() {
		java.util.Date date = new java.util.Date();
...
//				+ screenshotsFolder + "/" + nameWithExtention + "' width='200' height='100'/></a>");
	...
	private static void makeScreenshot2(WebDriver driver, String screenshotName) {
		WebDriver augmentedDriver = new Augmenter().augment(driver);
		/* Take a screenshot */
		File screenshot = ((TakesScreenshot) augmentedDriver).getScreenshotAs(OutputType.FILE);
		String nameWithExtention = screenshotName + ".png";
	 ...
	 */
	private static String getScreenshotLink(String screenshotName, String linkText) {
	...
	private static boolean isElemenEnabled(WebElement element) {
		try {
	...
	private static boolean isElementNotDisplayed(WebElement element) {
		try {
			return !element.isDisplayed();
		} catch (Exception e) {
			return false;
	 ...
	 */
	private static boolean waitForElementIsDisplayed(WebElement by) {
	 ...
	 */
	private static boolean isChecked(WebDriver driver, String locator) {
	 ...
	 */
	private static boolean isElementDisplayed(WebDriver driver, By elementLocator) {
		try {
	 ...
	 */
	private static void waitForElement1(WebDriver driver, String xpath) {
	...
	public static void waitForInvisibility(WebElement webElement, WebDriver driver) {
		(new WebDriverWait(driver, Duration.ofSeconds(25))).until(ExpectedConditions.invisibilityOf(webElement));
	}

	private static void presenceOfElement(By locator, WebDriver driver) {
		(new WebDriverWait(driver, Duration.ofSeconds(20))).until(ExpectedConditions.presenceOfElementLocated(locator));
	}

	private static void clickableElement(By locator, WebDriver driver) {
	...
	private static void waitForIframe(String str, WebDriver driver) {
		(new WebDriverWait(driver, Duration.ofSeconds(20)))
				.until(ExpectedConditions.visibilityOfElementLocated((By.cssSelector(str))));
	}

	private static void switchToIframeByCss(String str, WebDriver driver) {
	 ...
	 */
	private static String findAndSwitchToSecondWindow(WebDriver driver, String handleCurrentWindow) {
		pause(3);
	...
	private static void closeOtherWindow(WebDriver driver, String handleCurrentWindow) {
		Set<String> windows = driver.getWindowHandles();
		for (String window : windows) {
			if (!window.contains(handleCurrentWindow)) {
				driver.close();
	...
	private static String findAndSwitchToSecondWindows(WebDriver driver, String handleCurrentWindow,
	...
	private static void selectFromComboByIndex(WebElement element, int index) {
		Select select = new Select(element);
		select.selectByIndex(index);
	}

	private static String selectFromComboByIndexReturnValue(WebElement element, int index) {
		Select select = new Select(element);
		select.selectByIndex(index);
		pause(1);
	...
	private static String selectFromComboByIndexReturnValue(WebElement element) {
		Select select = new Select(element);
		return select.getFirstSelectedOption().getText();
	}
	...
	private static String getFridayDateOfParticularMonth(String year, String month) {
	 ...
	 */
	private static String getText(WebDriver driver, String elementName) {
		String text;
		try {
	 ...
	 * @return the text JS
	 */
	private static String getTextJS(WebDriver driver, WebElement element) {
	 ...
	 */
	private String getValue(WebDriver driver, String locator) {
	 ...
	 */
	private static boolean isElementDisplayed(WebDriver driver, String elementName) {
	 ...
	 * @param locator Locator of element.
	 */
	private static void waitForConditionIsElementPresent(WebDriver driver, String locator) {
		for (int second = 0;; second++) {
			if (second >= 10) {
				break;
			}
	 ...
	 */
	private static boolean isElementPresent(WebDriver driver, String locator) {
	 ...
	 * @return the int
	 */
	private static int randomNumericValueGenerate(int length) {
	...
	private static void type(WebDriver driver, WebElement webElement, String value) {
	 ...
	 */
	private static void waitForAjax(WebDriver driver, String num) {
		String ajax;
	 ...
	 */
	private static String ajaxFinised(WebDriver driver, String num) {
	 ...
	 * @throws InterruptedException the interrupted exception
	 */
	private static String selectRandomOptionFromCombo(By by, WebDriver driver) throws InterruptedException {
	 ...
	 */
	private static int getNumOfElements(WebDriver driver, By by) {
		int i = 0;
	 ...
	 * @param url    the url
	 */
	private static void openUrlInNewTab(WebDriver driver, String url) {
	 ...
	 */
	private static void mouseHover(WebDriver driver, WebElement ele) {
		Actions action = new Actions(driver);
		action.moveToElement(ele).build().perform();
	}

	private static void focusOnElement(WebDriver driver, WebElement ele) {
		Actions action = new Actions(driver);
	 ...
	 * @param ele    the element
	 */
	private static void mouseHoverUsingJS(WebDriver driver, WebElement ele) {
		String mouseOverScript = "if(document.createEvent){var evObj = document.createEvent('MouseEvents');evObj.initEvent('mouseover',true, false); arguments[0].dispatchEvent(evObj);} else if(document.createEventObject) { arguments[0].fireEvent('onmouseover');}";
		((JavascriptExecutor) driver).executeScript(mouseOverScript, ele);
	}

	private static void jsType(WebDriver driver, WebElement element, String value) {
	 ...
	 * @param driver the driver
	 */
	private static void goToPreviuosPage(WebDriver driver) {
	 ...
	 * @param driver the driver
	 */
	private static void stopPageLoading(WebDriver driver) {
		driver.findElement(By.tagName("body")).sendKeys(Keys.ESCAPE);
	}

	private static boolean isElementPresent(WebDriver driver, By identifier) {
		return !driver.findElements(identifier).isEmpty();
	}

	private static String getBeforeAndAfterDateMonthWise(int i) {
		Calendar cur = Calendar.getInstance();
	...
	private static String getBeforeAndAfterDateMonthWise(int i, int n) {
		Calendar cur = Calendar.getInstance();
		NumberFormat f = new DecimalFormat("00");
		cur.add(Calendar.MONTH, i);
	...
	private static String[] getBeforeAndAfterMonthYear(int i, int motnh, int year) {
		Calendar cur = Calendar.getInstance();
	...
	private static String getBeforeAndAfetrDateYearWis(int i) {
		Calendar cur = Calendar.getInstance();
		NumberFormat f = new DecimalFormat("00");
	...
	private static String getAbsolutePathForUpload(String document) {
		return new File(document).getAbsolutePath();
	}

	private static long dateCalculation(String addStart, String addEnd) {
	...
	private static int numberOfWeekDay(String addStart, String addEnd, String dayName) {
		int add = 0;
		try {
	...
	private static void switchtoTab(WebDriver driver, int tabNumber) {
		ArrayList<String> tabs = new ArrayList<>(driver.getWindowHandles());
		driver.switchTo().window(tabs.get(tabNumber));
	}

	private static String filePath(String filePath) {
		String finalPath;
	...
	private static void selectYear(WebDriver driver, WebElement dateBox, String year) {
